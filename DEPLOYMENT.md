...
5. **Build the public release bundle** validates the checkpoint and report,
   computes checksums, and creates `model-v2.0.0-release.zip`.
6. Download `conditioned-v2-best.pt` and the ZIP from the final notebook cell.
...
Copy `checkpoint.sha256` from `release-manifest-v2.json` in the ZIP. Keep the
tag immutable: publish a new tag for every later checkpoint instead of
...
variables to an existing Blueprint service):
...
MODEL_SHA256=<checkpoint.sha256 from release-manifest-v2.json>
